...
L.Control.Velocity = L.Control.extend({
  ...
  _onMouseMove: function _onMouseMove(e) {
    ...
    if (this.options.velocityType == "WIND") {
    ...
    } else if (this.options.velocityType == "WAVE") {
      var gridValue = this.options.leafletVelocity._windy.interpolatePoint(
        pos.lng,
        pos.lat
      );
      ...
      if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1])) {
        htmlOut =
          ...
          gridValue[0].toFixed(2) +
          "°" +
          ", <strong>Height: </strong>" +
          gridValue[1].toFixed(2) +
          "m" +
          ", <strong>Period: </strong>" +
          (!isNaN(gridValue[2]) ? gridValue[2].toFixed(2) + "s" : "No data");
      } else {
        htmlOut = this.options.emptyString;
      }
    }
...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _initWindy: function _initWindy(self) {
    ...
    this._windy =
      options.displayOptions.velocityType == "WIND"
        ? new Windy(options)
        : new Wavy(options); // prepare context global var, start drawing
...
var Wavy = function Wavy(params) {
  ...
  var PARTICLE_LINE_WIDTH = params.lineWidth || 10; // line width of a drawn particle

  var PARTICLE_MULTIPLIER = params.particleMultiplier || 1 / 200; // particle count scalar (completely arbitrary--this values looks nice)
  ...
  var NULL_WAVE_VECTOR = [NaN, NaN, null]; // singleton for no wave in the form: [u, v, magnitude]
  ...
  }; // interpolation for vectors like wave (direction, height, period)
  ...
  var bilinearInterpolateVector = function bilinearInterpolateVector(
    ...
    var a = rx * ry,
      ...
      c = rx * y,
      d = x * y;
    var dir = g00[0] * a + g10[0] * b + g01[0] * c + g11[0] * d;
    var ht = g00[1] * a + g10[1] * b + g01[1] * c + g11[1] * d;
    var prd = g00[2] * a + g10[2] * b + g01[2] * c + g11[2] * d;
    return [dir, ht, prd];
  };

  var createWaveBuilder = function createWaveBuilder(
    direction,
    height,
    period
  ) {
    var directionData = direction.data,
    ...
    return {
      ...
      data: function data(i) {
        return [directionData[i], heightData[i], periodData[i]];
      },
  ...
  var createBuilder = function createBuilder(data) {
    ...
    data.forEach(function(record) {
      switch (record.header.parameterNumberName) {
        ...
        case "Mean_period_of_wind_waves":
          period = record;
          break;
    ...
    return createWaveBuilder(direction, height, period);
  };

  var buildGrid = function buildGrid(data, callback) {
    var supported = true;
    if (data.length !== 3) supported = false;
    if (!supported)
      console.log(
        "Wavy Error: data must have 3 components (wave direction, height and perid)"
      );
    ...
    if (!supported) {
      console.log(
        "Wavy Error: Only data with Latitude_Longitude coordinates is supported"
      );
    }
  ...
  var distort = function distort(projection, λ, φ, x, y, scale, waveDir) {
    var u = -Math.sin((waveDir * τ) / 360) * scale;
    var v = Math.cos((waveDir * τ) / 360) * scale;
    var d = distortion(projection, λ, φ, x, y); // Scale distortion vectors by u and v, then add.

    var waveUV = [];
    waveUV[0] = d[0] * u + d[2] * v;
    waveUV[1] = d[1] * u + d[3] * v;

    var k = Math.sqrt(waveUV[0] * waveUV[0] + waveUV[1] * waveUV[1]);
    waveUV[0] = (VELOCITY * waveUV[0]) / k;
    waveUV[1] = (VELOCITY * waveUV[1]) / k;
    return waveUV;
  };
  ...
  var createField = function createField(columns, bounds, callback) {
    ...
    function field(x, y) {
      ...
      return (column && column[Math.round(y)]) || NULL_WAVE_VECTOR;
  ...
  var animate = function animate(bounds, field) {
    function waveIntensityColorScale(min, max) {
    ...
    var colorStyles = waveIntensityColorScale(
  ...
  var stop = function stop() {
    if (Wavy.field) Wavy.field.release();
...
var Windy = function Windy(params) {
  ...
  var PARTICLE_MULTIPLIER = params.particleMultiplier || 1 / 100; // particle count scalar (completely arbitrary--this values looks nice)
