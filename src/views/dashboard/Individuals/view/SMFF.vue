        ...
        >
          ...
          >
            ...
            <v-card-text class="text-center">
              ...
              />
              <v-img
                v-else-if="primaryCompanyHasPhoto"
                :src="`https://storage.googleapis.com/donjon-smit/pictures/companies/${primaryCompanyId}/cover_sqr.jpg`"
                class="img-avatar"
              />
              <v-icon
                v-else
        ...
        >
          ...
          >
            <base-material-alert
              ...
              dark
            >
              {{ userName }} Has No Assigned Capabilities.
            </base-material-alert>
          </base-material-card>
        </v-col>
        <v-col>
          <base-material-card
            color="primary"
            :title="userName + ' Has No Assigned Capabilities.'"
          >
            <v-card-text class="text-center">
              <v-img
                v-if="hasPhoto"
  ...
  export default {
    ...
    methods: {
      ...
      async getDataFromApi () {
        ...
        const res = await axios.get('users/' + this.$route.params.id + '/smff')
        this.networks = res.data.networks
        this.smff = res.data.smff
        this.serviceItems = res.data.serviceItems
        // Get User Plan Number
        const userData = await axios.get('users/' + this.$route.params.id)
        this.userName = `${userData.data.data[0].first_name} ${userData.data.data[0].last_name}`
        this.companyName = userData.data.data[0].primary_company ? userData.data.data[0].primary_company.name : `${this.userName} has no company assigned.`
        this.hasPhoto = userData.data.data[0].has_photo
        this.primaryCompanyHasPhoto = userData.data.data[0].primary_company && userData.data.data[0].primary_company.has_photo
        this.primaryCompanyId = userData.data.data[0].primary_company_id
        this.loading = false
      },
      createSMFF () {
      ...
      saveSMFF () {
        ...
        axios.post('users/' + this.$route.params.id + '/smff', data)
          .then(res => {
            this.notSaved = false
            this.getDataFromApi()
            this.showSnackBar({ text: res.data.message, color: 'success' })
